package org.api4.java.ai.ml.core.dataset.attribute;
...
public abstract class AAttributeValue<D> implements IAttributeValue<D> {
	...
	public void setValue(final D value) {
		if (!this.type.isValidValue(value)) {
			throw new IllegalArgumentException("The attribute value " + value + " does not conform the domain of the attribute type " + this.type.getClass().getName() + " with domain " + this.type.getStringDescriptionOfDomain() + ".");
		}
		this.value = value;
	}
